...
App({
  ...
  showComplexModal: function () {
    ...
    this.getStorage({
      ...
      success: function (res) {
        ...
        that.setData({ "popupDetails.cardNo": (res.data || 0).toString().replace(/(\d)(?=(?:\d{4})+$)/g, '$1 ')});
        that.animation = animation
        animation.translateY(that.data.animatiotnShowHeight).step()
        that.setData({
          "popupDetails.animationData": animation.export(),
          "popupDetails.showModalStatus": true
        });
  ...
  hideComplexModal: function () {
    var that = this.getAppCurrentPage();
    // 隐藏遮罩层  
    var animation = wx.createAnimation({
      duration: 200,
      timingFunction: "linear",
    ...
    setTimeout(function () {
      ...
      that.setData({
        "popupDetails.animationData": animation.export(),
        "popupDetails.showModalStatus": false
      })
    }.bind(that), 200)
  },
  onShowPopup: function (e) {
    ...
    wx.getSystemInfo({
      success: function (res) {
        that.setData({ animationShowHeight: res.windowHeight });
      }
    })
  },
